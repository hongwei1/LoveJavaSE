...
public class PassA
{
	...
	{
		long[] a1 = { 3, 4, 5 };
		long[] a2 = this.fix(a1); // a1[1] = 7 --> [3,7,5]
...
class SC2
{
	...
	void start()
	{
		...
		System.out.println(this.foo() + a + b + " "); // foo34
		System.out.println(a + b + this.foo());// 7foo
	}
...
/*
 * order is it right ?? implements Comparator< String> 1 /LoveJavaSE/src/itcastday19/p1/map/test/CompByLetter.java 2 TreeSetDemo.java 1
 * 判断元素唯一性的方式：就是根据比较方法的返回结果是否是0，是0，就是相同元素，不存。 2 TreeSet对元素进行排序的方式一： 让元素自身具备比较功能，元就需要实现Comparable接口,覆盖compareTo方法。 ComparatorByLength.java ComparatorByName.java
 * 3 TreeSet集合第二种排序方式二：(如果不要按照对象中具备的自然顺序进行排序。如果对象中不具备自然顺序。怎么办？) 让集合自身具备比较功能，定义一个类实现Comparator接口，覆盖compare方法。 将该类对象作为参数传递给TreeSet集合的构造函数。
 * 
 * --day18 集合技巧 1
 */
